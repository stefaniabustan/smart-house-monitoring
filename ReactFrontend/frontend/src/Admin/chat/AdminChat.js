...
export default function AdminChat() {
    ...
    const handleChatMessage = (receivedMessage, conversationKey = null) => {
        ...
        if (sender === loggedUser.username && key === "All Users") {
            //ignore your own messages in group chat
            return;
        }
    ...
    const filteredChats = [{ username: groupChatName }, ...users.filter(
                        ...
                        renderItem={(item) => (
                            <List.Item
                                className={`chat-list-item ${
                                    selectedRecipient === item.username ? 'active-chat' : ''
                                }`}
                                onClick={(e) => handleRecipientSelection(item.username)}
                            >
                                <List.Item.Meta
                                    avatar={
                    ...
                    <Header className="chat-header">
                        <Space className="header-name">
                            ...
                            {selectedRecipient === groupChatName && (
                                <Typography.Text>(Group Members: {users.map((u) => u.username).join(', ')})</Typography.Text>
                            )}
                        </Space>
                    </Header>
